...
- **세이프 영역(safe area)** — 잘려도 되는 가장자리. 인스타 4:5 크롭에서 위아래가 잘린다

우리 파이프라인은 3:4로 생성해 4:5로 맞추므로 **위아래가 약 2.7% 잘린다**(`fit_to_canvas`).
인물의 정수리와 발끝을 프레임 끝에 붙이면 잘린다.
...
템포를 만들고 독자가 한 번에 한두 컷만 본다. 인스타 캐러셀은 **4:5 카드 한 장이 완결**이고
